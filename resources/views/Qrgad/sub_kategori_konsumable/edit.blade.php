...
@section('content')
    <div class="card show">
        <div class="card-header">
            <h3><b>Edit Sub Kategori Konsumable</b></h3>
        </div>
        <div class="card-body">
            <div class="container"> 

                <form action="{{ url('/sub-kategori-konsumable') }}/{{ $sub_kategori_konsumable->id }}" method="post">
                    @method('put')
                    @csrf
                    <div class="form-group">
                        ...
                        value="{{ old('nama', $sub_kategori_konsumable->nama) }}" placeholder="Nama Sub Kategori Konsumable">
                        @error('nama')
                    ...
                    <div class="form-group">
                        ...
                        <select name="kategori_konsumable" id="kategori_konsumable" class="form-control @error('kategori_konsumable') is-invalid @enderror">
                            ...
                            @foreach ($kategori_konsumable as $k)    
                                <option value="{{ $k->id }}" {{ (old('kategori_konsumable', $sub_kategori_konsumable->kategori_konsumable) == $k->id) ? 'selected' : '' }}>{{ $k->nama }}</option>
                            @endforeach
                        </select>
